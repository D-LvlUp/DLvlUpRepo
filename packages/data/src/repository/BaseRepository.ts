...
export abstract class BaseRepository<T> {
  ...
  async GetAsync(query?: Func<Queryable<T>, Queryable<T>>) : Promise<any> {
    ...
    if(query != undefined) { return query(Querier).data[0]}
    else {return data[0]}  //Return first Result meeting the Criteria.
  }

  async GetRangeAsync(query?: Func<Queryable<T>, Queryable<T>>, distinct?: boolean) : Promise<any> {
    let data = await this._dataRepository.find()
    let Querier = new Queryable<T>(data);

    if(query != undefined) { return query(Querier).data}
    else {return data}  //Return Results meeting the Criteria.
  }
