...
export class APIService extends HttpClient {
    public constructor(baseUrl: string, private _authToken: string = '', private _apiKey: string = '') {
        super(baseUrl);
    ...
    private setService (authToken?: string, apiKey?: string): APIService {
        if(authToken != '') { this._headers.authToken = authToken }
        if(apiKey != '') { this._params.code = apiKey }
        return this
    }
    ...
    public get = <T>(endpoint: string, params: any) => this.instance.get<T>(endpoint, this.getRequest<T>(params));

    public post = <T>(endpoint: string, body: any) => this.instance.post<T>(endpoint, this.postRequest<T>(body))
