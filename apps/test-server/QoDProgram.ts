...
configuration_Process(async () => {
  ...
  await Program(DB, API, Mailer).then(async () => {
      await DB.destroy();
    }).catch(console.error);
...
async function Program(DB: DBContext, API: QoDApi, Mailer: MailerService) {
  if (checkInternetConnection() && DB.dbIsConnected()) {
      ...
      const ContactsInfo: Contacts[] = await contactsRepository.GetRangeAsync();

      const Subscribers: Subscriber[] = ContactsInfo.map(x => ({address: x.phone_email, name: x.Name}));

      Mailer.SubscriptionsList
          .addSubscription("QuotesOfDay")
